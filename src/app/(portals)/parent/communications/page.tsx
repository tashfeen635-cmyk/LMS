...
export default function ParentCommunicationsPage() {
  ...
  const { data: threads, isLoading: threadsLoading } =
    useThreadsByUser(userId || null);

  // Load courses for the first child to identify teachers
  const firstChildId = childrenIds[0] ?? null;
  const { data: childCourses } = useCoursesByStudent(firstChildId);
  ...
  // Get child's teachers (unique)
  const childTeachers = useMemo(() => {
    if (!childCourses) return [];
    const teacherMap = new Map<string, { teacherId: string; name: string; avatar: string; department: string; courses: string[] }>();
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Message threads or empty state */}
      {!threads || threads.length === 0 ? (
        ...
        /* Two-column layout */
        <div className="flex h-[calc(100vh-380px)] min-h-[400px] overflow-hidden rounded-lg border bg-card">
          {/* Thread list sidebar */}
          <div
            className={cn(
